...
uint8_t GPIO_ReadFromInputPin(GPIO_RegDef_t *pGPIOx, uint8_t PinNumber)
{
	...
	value = (uint8_t)((pGPIOx->IDR >> PinNumber) & 0x00000001);	//shift nth bit to LSB bit and mask with 0x00000001
...
{
	//1. First Find out Priority Register and section in priority register
	uint8_t iprx = IRQNumber / 4;
	uint8_t iprx_section = IRQNumber % 4;

	//Lower 4-bits are not Implement in each IPR section
	uint8_t shift_amount = (8 * iprx_section) + (8 - NO_PR_BITS_IMPLEMENTED); //8-4=4
...
{
	//1. Clear EXTI PR register corresponding to Pin number
